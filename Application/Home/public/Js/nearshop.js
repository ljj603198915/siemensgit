    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.code == 1) {
                ...
                $('.block-nearShop .shopKind1 .contain-addr').empty();
                $('.block-nearShop .shopKind2 .contain-addr').empty();
                $(data.on_activity1).each(function (index, ele) {
                    _html2 += '<a href="' + ele.shop_url + '"><div class="one-addr"><i><img src="/Application/Home/public/img/activity.png"></i><span class="span-shopName">' + ele.shop_name + '</span><br/><small class="span-shopAddr">地址：' + ele.shop_address + '</small></div></a>';
                    onLatlnts.push(ele.shop_latlnt)
                })
                $(data.on_activity2).each(function (index, ele) {
                    ...
                    noLatlnts.push(ele.shop_latlnt)
                })
...
//获取地图
function getMap(onActivity, noActivity) {
    //百度地图
    ...
    if (onActivity.length > 0) {
        //遍历加坐标
        for (var i = 0; i < onActivity.length; i++) {
            var arrPoint = onActivity[i].split(',')
            var point = new BMap.Point(Number(arrPoint[0]), Number(arrPoint[1]));
            addMarker(point);
        }
        // 编写自定义函数,创建标注
        function addMarker(point) {
            var myIcon = new BMap.Icon("/Application/Home/public/img/activity.png", new BMap.Size(32, 32));
    ...
    if (noActivity.length > 0) {
        ...
        for (var i = 0; i < noActivity.length; i++) {
            var arrPoint = noActivity[i].split(',')
            var point = new BMap.Point(Number(arrPoint[0]), Number(arrPoint[1]));
            addMarker(point);
        }
        // 编写自定义函数,创建标注
